...
- **AC-4 (glyph + ID only):** A bountied row renders its leading glyph as `◈` (`glyph.Bounty`) and its SPEC-ID in the theme's `Bounty` colour. The title, stage, assignee, and detail spans are **unchanged** from the non-bountied case, including their time-urgency ramp colour. A bountied, fully-stale row shows a gold spark and ID with a red title.
- **AC-5 (shimmer):** With `bounty.shimmer: true` (default) in an interactive TTY, a narrow specular highlight crosses the glyph+ID span periodically: one eased pass, then a rest, repeating, with each spec's phase offset by a hash of its ID so rows on screen do not pulse in unison. Consecutive frames within a pass produce different rendered strings; a non-bountied row's rendering is byte-identical across frames. With `shimmer: false`, the base shading remains and the highlight is parked off-span.
- **AC-5b (material shading):** The marker is shaded, not filled: at rest its tone falls monotonically from the glyph (brightest — it is the stone) to the last character of the ID, with enough range to read as a lit surface. The travelling highlight brightens whatever cell it lands on. On **every** shipped theme, including light and monochrome ones, the highlight's luminance is above the body's and the shadow's is below it — the highlight must never darken the marker.
- **AC-5c (finishes):** `bounty.finish` selects the metal: `gold` (default), `platinum`, or `prismatic`. The three are visibly distinct on a theme that can express them; `prismatic`'s highlight rotates through hue across a pass at constant perceived lightness. An unrecognised value is reported by lint and falls back to gold rather than failing to render. A theme with an explicit `BountyRamp` (monochrome `graphite`) overrides all three and stays achromatic.
...
- **2026-07-28 (node 2, glyph swap):** §4.1 and D-2 specified the boot-splash spark `✦` as the marker, for brand cohesion. In practice it rendered as a visibly smaller icon than the `●`/`◆`/`◐` it sits beside, and inconsistently between terminals. **Resolution:** a dedicated `glyph.Bounty` = `◈` (U+25C8), with the measurement and font-coverage reasoning recorded in D-17 and in the `internal/tui/glyph` package doc, plus `TestGlyphInkAndBlock` guarding the rule for future glyph choices. `✦` remains the boot-splash mark. Cohesion now rests on the gold colour, which nothing else in the TUI uses.
...
| D-2 | Which glyph marks a bounty? | (1) A new star glyph, (2) Reuse the boot-splash spark `glyph.Spark` (`✦`), (3) Emoji | **DECIDED: (2) `glyph.Spark`, then REVISED to (1) `glyph.Bounty` = `◈` (U+25C8)** | Originally chosen for brand cohesion with the boot splash. **Superseded after pilot feedback (see D-17):** `✦` is a Dingbat, and measurement showed it carries ~24% of the ink of `●` in a real terminal font, so it read as a tiny icon beside its neighbours — and mainstream programming fonts do not cover the Dingbats block at all. The row marker moved to Geometric Shapes; the splash keeps `✦`, where it has no neighbours to be dwarfed by. Emoji remains barred by the no-emoji snapshot invariant | Owner | 2026-07-28 |
| D-17 | The bounty glyph rendered inconsistently across terminals — which glyph, and why? | (1) Keep `✦` and accept the variance, (2) Keep `✦` and add a user-level glyph override, (3) Move the row marker into Geometric Shapes (U+25xx) | **DECIDED: (3) `◈` (U+25C8) for the row marker; `✦` stays on the splash** | Owner reported `✦` rendering small in Alacritty but correct in Zed's terminal. Measured, not guessed: in `MesloLGLDZ Nerd Font Mono` the outline of `✦` has **24%** of the ink area of `●` (`◆` 63%, `◐` 68%, `▲` 63%), and its bounding box is 56% em against 60% — so it is genuinely lighter, and how much lighter depends on the font, which is why two terminals disagreed. Worse, `fc-list` shows JetBrains Mono, Caskaydia, and Noto Sans Mono cover **no** Dingbats, so those users get a fallback font at a fallback size. `◈` sits at 50% ink, is covered by every one of those fonts, is an unused shape, and reads as a gem — which suits a bounty as well as a spark did. A user-level override was rejected as pushing a font problem onto every user instead of fixing the default | Owner | 2026-07-28 |
| D-18 | The marker read as flat coloured text, not something precious — how do we make it alluring? | (1) A costlier metal (platinum/rhodium), (2) Shade the marker as a lit material and fix the highlight, (3) Escalate through loot-rarity hues | **DECIDED: (2) render the material properly; expose `bounty.finish` = gold \| platinum \| prismatic** | Owner was unsatisfied with the look. Measurement found two defects, not a wrong metal: the highlight blended toward `Theme.Text`, so it **darkened** the marker on `github-light` (lum 38→22), `modus-operandi` (38→11) and `graphite` (100→89), and merely desaturated gold to beige on dark themes; and the body was a flat fill, which reads as coloured text rather than a surface. The fix is a three-tone finish (shadow/body/specular), a base gradient falling away from the glyph, a tight squared-falloff specular blending toward **light**, and a per-spec phase offset so rows twinkle independently. On "something more valuable": the materials ladder (silver → gold → platinum → rhodium → diamond) runs toward achromatic, and achromatic in a terminal is just primary text — so platinum is *less* alluring here, which is why games escalate rarity through saturated hue instead. `prismatic` is the genuine escalation: a near-white stone whose highlight rotates hue in HCL (constant perceived lightness, no yellow-bright/blue-dark lurch). All three ship so the choice can be made by eye | Owner | 2026-07-28 |
...
14. ~~**Which glyph, given per-terminal rendering differences:**~~ see **D-17** — `◈` from Geometric Shapes, measured on ink mass and font coverage.

**Still open:**

15. **Bounty expiry:** should an active bounty expire (end of cycle, or a configurable `expires_after`) so an abandoned plan doesn't hold a cap slot forever? Not implemented: `max_active` already forces the granter back to the list, and expiry adds a clock to reason about. Revisit if the pilot shows stale gold.
16. **Search-result rows:** deferred with a recorded reason (§8). Fold `bountied` into the next `spec_search` schema migration that happens for a substantive reason.
17. **Pilot question — does the marker land on the right work?** The one risk the tool cannot enforce (§7.2): bounties belong on unglamorous critical work, not on the appealing greenfield spec. Grant/clear churn is in the activity log; review it after a cycle and decide whether the usage principle needs teeth (e.g. a warning when a bountied spec is also the newest spec in the pipeline).
