...
func TestCheckAgentGenerate_MeasuresRoundTrip(t *testing.T) {
	...
	if err := checkAgentGenerate(client, &report); err != nil {
		t.Fatalf("checkAgentGenerate: %v", err)
	}
...
func TestCheckAgentGenerate_EmptyResponseFails(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	var report agentReport
	err = checkAgentGenerate(client, &report)
	if err == nil {
...
func TestCheckAgentReachable(t *testing.T) {
	tests := []struct {
		name      string
		cfg       config.ProviderConfig
		wantErr   string
		wantField func(agentReport) string
	}{
		{
		...
		},
		{
			...
			wantField: func(r agentReport) string { return r.Endpoint },
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var report agentReport
			err := checkAgentReachable(tt.cfg, &report)
...
func TestInertGenerateSettings(t *testing.T) {
	...
	if got := inertGenerateSettings("anthropic", api); len(got) != 0 {
...
// that exist.
func TestDetectedHarnesses_OnlyReportsInstalled(t *testing.T) {
...
func TestAgentCheck_ProbeUsesProductionContainment(t *testing.T) {
	...
	var report agentReport
	if err := checkAgentGenerate(stub, &report); err != nil {
		t.Fatalf("checkAgentGenerate: %v", err)
	}
	if recorded.Task == "" {
